...
const Tasktable = ({ rows, onEditTask, onDeleteTask }) => {
    ...
    const handlePrint = () => {
        ...
        doc.text('Microservice Center', 14, 50); // Adjust the text position as needed

        // Contact Information
        doc.setFontSize(12);
        doc.setTextColor(0, 0, 0);
        const date = new Date().toLocaleDateString();
        doc.text(`Date: ${date}`, 160, 50); // Date aligned to the right
        ...
        doc.setFontSize(18);
        doc.text('Task Report', 70, 80); // Adjust the text position as needed

        // Add total, completed, and ongoing tasks summary
        doc.setFontSize(12);
        doc.text(`Total Tasks: ${totalTasks}`, 14, 90);
        doc.text(`Completed Tasks: ${completedTasks}`, 14, 98);
        doc.text(`Ongoing Tasks: ${ongoingTasks}`, 14, 106);

        // Add table of tasks
        doc.autoTable({
            startY: 120, // Start after the summary
