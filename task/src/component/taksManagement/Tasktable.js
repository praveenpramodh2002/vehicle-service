...
const Tasktable = ({ rows, onEditTask, onDeleteTask }) => {
    ...
    const handlePrint = () => {
        ...
        doc.setFontSize(18);
        doc.text('Task Report', 70, 30); // Adjust the text position to account for the logo

        // Add total, completed, and ongoing tasks
        doc.setFontSize(12);
        doc.text(`Total Tasks: ${totalTasks}`, 14, 50);
        doc.text(`Completed Tasks: ${completedTasks}`, 14, 58);
        doc.text(`Ongoing Tasks: ${ongoingTasks}`, 14, 66);

        // Add table of tasks
        doc.autoTable({
            startY: 80, // Start after the summary
